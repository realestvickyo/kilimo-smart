...
const MapView: React.FC<{ farmer: FarmerProfile }> = ({ farmer }) => {
  ...
  const mapUrl = `https://api.mapbox.com/styles/v1/mapbox/light-v10/static/35.8,-1.0,7/800x600?access_token=${MAPBOX_TOKEN}`;
    ...
    <div className="space-y-6 pb-12">
      <div className="flex items-center justify-between mb-2">
        <div>
          ...
          <p className="text-stone-400 text-sm font-medium">
            Monitoring {selectedCounty.name} for pest and climate risks.
          </p>
        </div>
        ...
        <div className="bg-white px-4 py-2 rounded-xl border border-stone-100 shadow-sm flex items-center gap-2">
          <span
            className={`w-3 h-3 rounded-full animate-pulse ${
              selectedCounty.risk === 'High' ? 'bg-red-500' : 'bg-emerald-500'
            }`}
          ></span>
          <span className="text-xs font-black uppercase">
            {selectedCounty.risk} Risk Status
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 bg-white rounded-[2.5rem] p-4 shadow-xl border border-stone-100 relative h-[60vh] overflow-hidden">
          ...
          <div
            className="absolute inset-0 bg-stone-100 bg-cover bg-center opacity-60"
          ...
          {KENYA_COUNTIES.map((county, i) => (
            ...
            >
              <div
                className={`p-3 rounded-2xl shadow-2xl flex items-center gap-3 transition-all active:scale-95 ${
              ...
              >
                <div
                  ...
                  style={{
                    backgroundColor: county.color + '20',
                    color: county.color,
                  }}
                >
            ...
            <div className="bg-white/95 backdrop-blur-md p-6 rounded-3xl border border-stone-100 shadow-2xl flex items-center justify-between">
              ...
              {!farmer.isPremium && (
                <div className="text-right hidden sm:block">
                  <p className="text-[10px] text-amber-600 font-black mb-1 uppercase">
                    PRO FEATURE
                  </p>
                  <button className="text-emerald-600 text-xs font-black underline">
                    Unlock Heatmaps
                  </button>
                </div>
              )}
        ...
        <div className="lg:col-span-4 space-y-6">
          <section className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-stone-100">
            <h3 className="font-bold text-lg mb-6">
              Risk Details: {selectedCounty.name}
            </h3>
            ...
            <div className="space-y-4">
              ...
              <div className="p-5 bg-stone-50 rounded-3xl border border-stone-100">
                ...
                <p className="text-sm font-medium text-stone-600">
                  {selectedCounty.risk === 'High'
                    ? 'Apply pyrethroid-based pesticide in late evening.'
                    : 'Continue regular irrigation cycle.'}
                </p>
              </div>

              <button className="w-full bg-stone-900 text-white py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg shadow-stone-100 transition-transform active:scale-95">
                Report New Hazard
              </button>
            </div>
          </section>
          ...
          <div className="bg-emerald-700 text-white p-8 rounded-[2.5rem] shadow-xl relative overflow-hidden group">
            ...
            <p className="text-sm text-emerald-100/70 mb-6 font-medium leading-relaxed">
              Finding verified buyers within 50km of {selectedCounty.name}...
            </p>
